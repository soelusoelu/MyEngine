...
float Easing::easeOutCubic(float value) {
    return 1.f - Math::pow(1.f - value, 3);
}

float Easing::easeInOutCubic(float value) {
    return value < 0.5f ? 4.f * value * value * value : 1.f - Math::pow(-2.f * value + 2.f, 3) / 2.f;
}
...
float Easing::easeOutBack(float value) {
    ...
    return 1.f + C3 * Math::pow(value - 1.f, 3) + C1 * Math::pow(value - 1.f, 2);
}
...
float Easing::easeInOutBack(float value) {
    ...
    return value < 0.5f
        ? (Math::pow(2.f * value, 2) * ((C2 + 1.f) * 2.f * value - C2)) / 2.f
        : (Math::pow(2.f * value - 2.f, 2) * ((C2 + 1.f) * (value * 2.f - 2.f) + C2) + 2.f) / 2.f;
}
